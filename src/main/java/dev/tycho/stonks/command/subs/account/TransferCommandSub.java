...
public class TransferCommandSub extends ModularCommandSub {
  ...
  public void execute(Player player) {
    ...
    new CompanySelectorGui.Builder()
        .companies(list)
        .title("Select a company to transfer from")
        .companySelected((company -> {
          //Cache the next screen
          new AccountSelectorGui.Builder()
              .company(company)
              .title("Select which account to transfer from")
              .accountSelected(transferFrom -> payCompany(player, transferFrom, message, amount))
              .show(player);
        }))
        .show(player);
  }

  private void payCompany(Player sender, Account transferFrom, String message, double amount) {
    List<Company> list = Repo.getInstance().companies().getAll();
    new CompanySelectorGui.Builder()
        .companies(list)
        ...
        .companySelected((company -> {
          ...
          new AccountSelectorGui.Builder()
              ...
              .accountSelected(transferTo -> {
                ...
                sendMessage(sender, "Paid " + ChatColor.YELLOW + company.name + " (" + transferTo.name + ")" + ChatColor.YELLOW + " $" + Util.commify(amount) + ChatColor.GREEN + "!");
                ...
                Repo.getInstance().sendMessageToAllOnlineManagers(
                    company, sender.getDisplayName() + ChatColor.GREEN + " paid " + ChatColor.YELLOW + " " + company.name + " (" + transferTo.name + ") $" + Util.commify(amount));
              })
              .show(sender);
        }))
        .show(sender);
